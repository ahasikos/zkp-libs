...
impl Polynomial {
    ...
    pub fn new(coeffs: &Vec<FieldElement>, var: char) -> Self {
        let modified_coeffs = list::remove_trailing_elements(&coeffs, &FieldElement::zero());
    ...
    pub fn degree(&self) -> usize {
        if self.poly.len() == 0 {
            ...
            return list::remove_trailing_elements(&self.poly, &FieldElement::zero()).len() - 1;
        }
    }      
    ...
    pub fn scalar_mul(&self, scalar: &FieldElement) -> Polynomial {
        Polynomial { poly: list::scalar_multiplication::<FieldElement>(&self.poly, scalar), var: 'x' }
    }
    ...
    pub fn interpolate_poly(x_values: &Vec<FieldElement>, y_values: &Vec<FieldElement>) -> Polynomial {
        ...
        let ret = list::remove_trailing_elements(&poly.poly, &FieldElement::zero());
...
impl std::ops::Add for Polynomial {
    ...
    fn add(self, rhs: Self) -> Self::Output {
        Polynomial {
            poly: list::two_lists_tuple_operation(&self.poly, &rhs.poly, |a, b| a.clone() + b.clone()), var: self.var
        }
    }
}
